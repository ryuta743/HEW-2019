...
export const actions = {
    async get_contactdata({commit}){
        console.log('僕のかち');
        const all_contactdata = await this.$axios.$get(`http://133.18.194.128:5000/contact/admin_contact`);
        console.log(all_contactdata)
        commit('setContactdata',all_contactdata)
    },
    async get_contact_detail({commit},{contact_data}){
        console.log('うわああああああ')
        const contact_id = contact_data.contact_id
        const contact_detail = await this.$axios.$get(`http://133.18.194.128:5000/contact/admin_contact_detail?contact_id=${contact_id}`);
